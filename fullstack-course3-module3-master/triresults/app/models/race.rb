class Race
  ...
  scope :upcoming, ->{where(:date.gte=>Date.current)}
	...
	def next_bib 
		#perform an atomic increment of the next_bib value in the database and return the result of next_bib
		res = inc(next_bib: 1)
		res[:next_bib]
	...
	def get_group racer
		...
		Placing.demongoize(:name=>name) end
	end
	...
	def self.upcoming_available_to racer
		...
		all_races_ids = Race.upcoming.map {|r| r[:_id]}
		#We make the difference between the 2 sets
		races_upcomming_diff = all_races_ids - upcoming_race_ids
		Race.in(:id => races_upcomming_diff)
